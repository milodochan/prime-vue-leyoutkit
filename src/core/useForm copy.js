	...
	const rowApi = {
		setColumn(field, callback, label = "") {
			...
			const columnApi = {
				...
				setPlaceholder(text) {
					column.fieldAttr.placeholder = text
					return columnApi
				},
				onRequire() {
					column.fieldAttr.require = true
					return columnApi
				},
				setAttr(attrs = {}) {
...
export function useForm() {
	...
	const register = () => {
		...
		const setData = (formData) => (data = formData)
		forms.value.push({ data, config })

		return { setRow, setData }
	}
